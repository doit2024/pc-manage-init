...
import { LOGIN_INFO, USER_INFO, ACCESS_TOKEN } from '../../php/project'
    ...
    if (res.errcode === 0) {
      ...
      if (whereCatch === 'local') {
        ...
        if (res.errcode === 400006) {
          ...
          let content = ((Date.now() - lastLoginInfo.login_time) < 1000 * lastLoginInfo.expire)
          ? '您的账号在异地登录，是否强制登录？'
          : '凭证失效， 是否重新登录？'
          Vue.prototype.$Modal.confirm({
            ...
            onOk: () => {
              ...
              if (userinfo) {
                Vue.prototype.$Message.success('重新登录成功！')
                Vue.prototype.$http.account.login(JSON.parse(aes.decrypt(userinfo))).then(data => {
...
const _ajaxOptions = {
  ...
  timeout: 10000,
  data: {},
  noNull: true,  // 参数属性值是否不允许为 ['' | undefined | null]
  dataType: 'text',
  hasAccessToken: true,
