...
const _formateResData = (res, url, whereCatch, resolve, reject) => {
  ...
  if (res.errcode === 0) {
    ...
    if (whereCatch === 'local') {
      ...
      const lastLoginInfo = JSON.parse(aes.decrypt(window.localStorage.getItem(LOGIN_INFO)))
      const isLoginTime = ((Date.now() - lastLoginInfo.login_time) < 1000 * lastLoginInfo.expire)
      isLoginTime && Vue.prototype.$Notice.warning({ title: '账号异地登录!' })
